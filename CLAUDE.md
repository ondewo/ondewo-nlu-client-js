...
- **`RELEASE.md` and `src/RELEASE.md` must stay byte-identical, and `src/` is the authoritative one.** `make build` does `cp src/RELEASE.md .`, so any edit applied only to the root file is silently reverted by the next release. (This is exactly how root drifted deduped while `src/` kept 5 duplicate headings.) Same rule for `README.md` / `src/README.md` — and there the release additionally computes the `START/END OF GITHUB README` cut range from `src/README.md` but applies it to a copy of the ROOT file, so a one-sided edit truncates the published README. Verify with `diff RELEASE.md src/RELEASE.md` and `diff README.md src/README.md` before committing.
- **Version bumps touch four files.** `ONDEWO_NLU_VERSION` in the `Makefile`, `version` in `package.json` and `src/package.json`, and `package-lock.json` (via `npm install`, never by hand). A version with no matching `## Release ONDEWO NLU Js Client X.Y.Z` heading makes `gh release create -n "$(CURRENT_RELEASE_NOTES)"` publish EMPTY notes — check with `make TEST`, and check `make spc` passes before releasing.
...
This repo has submodules (`ondewo-proto-compiler`, `src/ondewo-nlu-api`). If a submodule's working
...
```text
Release ONDEWO NLU Js Client ${ONDEWO_NLU_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO NLU Js Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO NLU Js Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/ondewo-nlu-client-js`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/ondewo-nlu-client-js versions --json
```
