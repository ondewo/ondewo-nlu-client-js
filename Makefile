...
IMAGE_UTILS_NAME=ondewo-nlu-client-utils-js:${ONDEWO_NLU_VERSION}
PRETTIER_WRITE?=

CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO NLU Js Client ${ONDEWO_NLU_VERSION}/../^\*{5}/'`
...
check_build: ## Checks if all built proto-code is there
	...
	do \
		cat api/ondewo_nlu_api.js | grep -q $${file};\
		if test $$? != 0; then  echo "No Proto-Code for $${file} in api/ondewo_nlu_api.js" & exit 1;fi \
	done
	@rm -rf build_check.txt
...
update_package: ## Updates Package Version in src/package.json
	@perl -i -pe "s/\"version\": \"[0-9]*.[0-9]*.[0-9]\"/\"version\": \"${ONDEWO_NLU_VERSION}\"/g" src/package.json
